...
(function ($) {
        ...
        if ($['validator'] && $['validator']['methods']) {
            ...
            $['validator']['methods']['date'] = function (value, element) {
                ...
                if (format) {
                    var date = null;
                    try {
                        date = $.datetimepicker.parseDate(format, value);
                    }
                    catch (e) { }
    ...
    function initJQueryUI(elements) {
        var dpc = elements.filter('[data-jqui-type="datetimepicker-culture"]').first();
        if (dpc.length) {
            $['datetimepicker']['setDefaults']($['datetimepicker']['regional'][dpc.val()]);
        }
        ...
        elements.filter('[data-jqui-type="datetimepicker"]').each(function () {
            ...
            ['numberOfMonths']);
            setupdatetimepickerUniqueAttributes(dp, options);
            dp.datetimepicker(options);
            initdatetimepicker(dp);
        });
    ...
    function setupdatetimepickerUniqueAttributes(element, options) {
    ...
    function initdatetimepicker(element) {
        ...
        if (valueSelector) {
            var valueElement = $('#' + valueSelector);
            if (valueElement.val()) {
                element.datetimepicker('setDate', valueElement.val());
            }
        }
    }
